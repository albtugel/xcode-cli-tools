...
xrun() {
  ...
  local keys=(iph pro pm plus se ipadpro air)
  if [[ -z "$arg" ]]; then
    echo "${C}--- Device ---${N}"
    local i=1; for k in "${keys[@]}"; do echo "  $i) $k"; ((i++)); done
    echo -n "${C}Choice [1-7]:${N} "; read -r dc; arg="${keys[$((dc))]}"
  fi
    ...
    for rt, devs in data.get('devices', {}).items():
        ...
        for d in devs:
            ...
            if (target=='iph' and 'iphone' in n and not any(x in n for x in ['pro','max','plus','se'])) or \
               (target=='pro' and 'pro' in n and 'max' not in n) or \
               (target=='pm' and 'pro max' in n) or \
               (target=='plus' and 'plus' in n) or \
               (target=='se' and 'se' in n):
...
")
  ...
  if xcodebuild build $flag "$abs_p" -scheme "$scheme" -destination "platform=iOS Simulator,id=$device_id" -sdk iphonesimulator CODE_SIGNING_ALLOWED=NO 2>&1 | \
    sed -e "s/.*BUILD SUCCEEDED.*/${G}** BUILD S\U\C\C\E\E\D\E\D **${N}/gi" \
